...
class RoadNetwork(object):
    ...
    def get_closest_lane_index(self, position: np.ndarray) -> LaneIndex:
        ...
        test=indexes[int(np.argmin(distances))]
    ...
    def next_lane(self, current_index: LaneIndex, route: Route = None, position: np.ndarray = None,
                  np_random: np.random.RandomState = np.random , high_priority_vehicle=False) -> LaneIndex:
        """
        ...
        if not next_to:
            try:
                next_lanes_list=[]
                for k,v in (self.graph[_to]).items():
                    test_lane = self.graph[_to][k][0]
                    if test_lane.forbidden == False or high_priority_vehicle:
                        next_lanes_list.append(k)
                # next_to = next_lanes_list[np_random.randint(len(next_lanes_list))]
                if len(next_lanes_list) == 0:
                    return current_index
                pick_lane=np_random.randint(len(next_lanes_list))
                next_to = next_lanes_list[pick_lane]
...
class Road(object):
    ...
    def __init__(self,
                 ...
                 record_history: bool = False, training = True , options = None) -> None:
        """
        New road.
    ...
    def close_vehicles_to(self, vehicle: 'kinematics.Vehicle', distance: float, count: int = None,
                          see_behind: bool = True, order="sorted") -> object:
        ...
        elif order == "sorted_by_id":
            vehicles = sorted(vehicles, key=lambda v: abs(v.id))
        elif order == "sorted_by_x":
            # TODO test sorted by x
    ...
    def step(self, dt: float) -> None:
        ...
        # print("point2.1 diff = {:.3f} ms".format((time.time() - start) * 1000))
        ...
        for vehicle in self.vehicles:
            ...
            for other in self.objects:
                vehicle.check_collision(other)
        # print("point2.2 diff = {:.3f} ms".format((time.time() - start) * 1000))
        # for each vehicle, check distance to the vehicle in front of them

        # if not self.training:
        # if self.options:
        #     if self.options['--create_timestep_log']:
        #         for vehicle in self.vehicles:
        #             distance, _  = self.distance_to_front_vehicle(vehicle)
        #             vehicle.distance_to_front = distance
        # print("point2.3 diff = {:.3f} ms".format((time.time() - start) * 1000))

    def distance_to_front_vehicle(self, vehicle):
        """
        Finds the distance of a vehicle to its neighbor vehicle in front on vehicle's lane

        :param vehicle: the vehicle whose neighbours must be found

        :return: distance to vehicle in front and the vehicle in front
        """
        ...
        if s_front is not None:
            distance_to_front = s_front - s
            assert (distance_to_front >= 0), 'distance_to_front should always be a positive value'

        return distance_to_front, v_front

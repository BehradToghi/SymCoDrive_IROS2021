...
class WorldSurface(pygame.Surface):
    ...
    """
    mode=1: plot only lane boundaries
    mode=2: fill the drivable area, 
    """
...
class LaneGraphics(object):
    ...
    def display(cls, lane: AbstractLane, surface: WorldSurface) -> None:
        ...
        if surface.mode == 1:
            ...
            thickness = 0.3
            line_color = surface.GREEN
            rect_color = surface.YELLOW

            p1 = surface.vec2pix(lane.start - [0, lane.width / 2])
            p2 = surface.vec2pix(lane.start + [0, lane.width / 2])
            p3 = surface.vec2pix(lane.end - [0, lane.width / 2])
            p4 = surface.vec2pix(lane.end + [0, lane.width / 2])
            points = [p1, p2, p4, p3]

            pygame.draw.polygon(surface, rect_color, points)
    ...
    def draw_stripes(cls, lane: AbstractLane, surface: WorldSurface,
        ...
        #         if abs(starts[k] - ends[k]) > 0.5 * cls.STRIPE_LENGTH:
        #
        #             # left = (surface.vec2pix(lane.position(starts[k], lats[k])))[0]
        #             # top = (surface.vec2pix(lane.position(starts[k], lats[k])))[1]
        #             # width = abs(surface.pix(ends[k]-starts[k]))
        #             # height = abs(surface.pix(lats[k]))

...
class AbstractEnv(gym.Env):
    ...
    def __init__(self, config: dict = None) -> None:
        ...
        self.merged_vehicle = False  # True when merge_vehicle merge to highway
        self.merged_counter = 0
        self.actions = None  # Actions that a vehicle can take, dict
        self.actions_indexes = None  # Actions indexes
    ...
    def _info(self, steps):
        """
        logs the info here. e.g., speed, collisions, etc.
        """
        raise NotImplementedError
    ...
    def step(self, action: Action) -> Tuple[Observation, float, bool, dict]:
        ...
        # run the simulation, step for each vehicle and road
        self._simulate(action)
        if self.timing:
            simulate_elapsed_time = 1000 * (time.time() - simulate_start_time)
            print("INFO: -----Simulation time + {:.1f}ms".format(simulate_elapsed_time))

        if self.timing:
            observation_start_time = time.time()
        # get last state/observation
        obs = self.observation_type.observe()
        # check if the mission is accomplished
        if self.timing:
            observation_elapsed_time = 1000 * (time.time() - observation_start_time)
            print("INFO: -----Observation time + {:.1f}ms".format(observation_elapsed_time))
        ...
        # print(">>>>>>>>>>>>>>>>>>>>>>>mission_accomplished: ", self.mission_accomplished)
        ...
        if self.timing:
            reward_elapsed_time = 1000 * (time.time() - reward_start_time)
            print("INFO: -----Reward time + {:.1f}ms".format(reward_elapsed_time))
        # unroll info from reward if dict of independent rewards
        # if isinstance(reward[0],dict):
        #     reward_tuple = tuple(rewardx["reward"] for rewardx in reward)
        #     reward_info = reward
        #     reward = reward_tuple
        # else:
        #     reward_info = {}
        ...
        # TODO: Behrad: should complete this info logger
        info = self._info(self.steps)

        # TODO: add cost to info too (like below)
        # info = {
    ...
    def _simulate(self, action: Optional[Action] = None) -> None:
        ...
        for _ in range(int(self.config["simulation_frequency"] // self.config["policy_frequency"])):
            ...
            # self.time += 1
            # if timing:
            #     road_elapsed_time = 1000 * (time.time() - road_start_time)
            #     print("INFO: -------Road time + {:.1f}ms".format(road_elapsed_time))
            #
            # if timing:
            ...
            # if timing:
            #     render_elapsed_time = 1000 * (time.time() - render_start_time)
            #     print("INFO: -------Render time + {:.1f}ms".format(render_elapsed_time))
    ...
    def render(self, mode: str = 'human') -> Optional[np.ndarray]:
        ...
        if mode == 'heatmap_array':
            image = self.viewer.get_heatmap()
        ...
        if mode == 'rgb_array':
            ...
            self.image = image
            # plt.imsave("/home/rodolfo/PycharmProjects/coop_repo/images/test.png", image)

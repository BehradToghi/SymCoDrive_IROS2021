...
class DiscreteMetaAction(ActionType):
    ...
    def __init__(self,
        ...
        self.lateral = lateral
        self.stop =stop
        actions = self.ACTIONS_ALL if longitudinal and lateral \
            else self.ACTIONS_LONGI if longitudinal \
            else self.ACTIONS_LAT if lateral \
            else self.ACTIONS_IDLE
        # TODO: Behrad: this is to test agents that only idle
        # if self.actions is None:
        #     raise ValueError("At least longitudinal or lateral actions must be included")
        self.actions = copy.deepcopy(actions)
        if self.stop:
            self.actions[len(self.actions)] = 'STOP'
...
class MultiAgentAction(ActionType):
    def __init__(self,
        ...
        for idx, vehicle in enumerate(self.env.controlled_vehicles):
            action_type = action_factory(self.env, self.action_config)
            action_type.controlled_vehicle = vehicle
            self.env.actions = action_type.actions
            self.env.actions_indexes = action_type.actions_indexes

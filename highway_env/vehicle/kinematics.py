...
class Vehicle(object):
    ...
    # def save_action_history(self, action):
    #
    #     # self.action_history.appendleft(action)
    #     a = self.actions_indexes[action] + 1
    #     a=a/len(self.actions_indexes)
    #     self.action_history.appendleft(a)
    ...
    def to_dict(self, origin_vehicle: "Vehicle" = None, observe_intentions: bool = True,
                action_history_count=0, action_history_type="discrete", observation=None) -> dict:
        d = {
            'presence': 1,
            'x': self.position[0],
            'y': self.position[1],
        ...
        if action_history_count > 0:
            if action_history_type == "discrete":
                for i in range(action_history_count):
                    a = 'a' + str(i)
                    if i < len(self.action_history):
                        action = self.action_history[i]
                        value = observation.env.actions_indexes[action]
                        value_norm = value / (len(observation.env.actions_indexes) - 1)
                        d[a] = value_norm
                    else:
                        d[a] = 0
            elif action_history_type == "binary":
                for i in range(action_history_count):
                    for key, action in observation.env.actions.items():
                        #
                        # for idx, action in enumerate(self.observer_vehicle.actions):
                        a = 'a' + str(i) + '_' + action
                        if i < len(self.action_history):
                            d[a] = 1 if self.action_history[i] == action else 0
                        else:
                            d[a] = 0
            elif action_history_type == "xy_discrete":
                for i in range(action_history_count):
                    ...
                    ax_val, ay_val = (0, 0)
                    if i < len(self.action_history):
                        ax_val, ay_val = self.xy_discrete_map[self.action_history[i]]
                    d[ax] = ax_val
                    d[ay] = ay_val

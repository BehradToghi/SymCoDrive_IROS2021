...
class ControlledVehicle(Vehicle):
    ...
    def __init__(self,
                 ...
                 config={},
                 id =0):
        super().__init__(road, position, heading, speed,id)
        self.config=config
    ...
    def follow_road(self,high_priority_vehicle=False) -> None:
        """At the end of a lane, automatically switch to a next one."""

        getlane=self.road.network.get_lane(self.target_lane_index)
        # test_after_end=test_getlane.after_end(self.position)
        if self.road.network.get_lane(self.target_lane_index).after_end(self.position):
            self.target_lane_index = self.road.network.next_lane(self.target_lane_index,
                                                                 ...
                                                                 np_random=self.road.np_random,high_priority_vehicle=high_priority_vehicle)
...
class MDPVehicle(ControlledVehicle):
    ...
    def __init__(self,
                 ...
                 target_lane_index: LaneIndex = None,
                 target_speed: float = None,
                 route: Route = None, config={}, id=0, min_speed=15, max_speed=35 ) -> None:
        super().__init__(road, position, heading, speed, target_lane_index, target_speed, route, config,id)
        self.min_speed = min_speed
        self.max_speed = max_speed

        #TODO clean , remove min_speed, keep SPEED_MIN
    ...
    def index_to_speed(self, index: int) -> float:
        ...
        if self.SPEED_COUNT > 1:
            return self.min_speed + index * (self.max_speed - self.min_speed) / (self.SPEED_COUNT - 1)
        else:
    ...
    def speed_to_index(self, speed: float) -> int:
        ...
        """
        x = (speed - self.min_speed) / (self.max_speed - self.min_speed)
    ...
    def speed_to_index_default(cls, speed: float) -> int:
        ...
        :return: the index of the closest speed allowed []
        """
        x = (speed - cls.min_speed) / (cls.max_speed - cls.min_speed)
